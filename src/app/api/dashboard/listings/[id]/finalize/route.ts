    ...
    try {
      ...
      if (sellerEmail) {
        ...
        if (!sellerResult.sent) {
          const sellerErr = sellerResult.error ?? "Seller email failed.";
          emailError = emailError ? `${emailError}; ${sellerErr}` : sellerErr;
      ...
      console.error("[listing-finalize] email dispatch failed:", emailError);
    }
  }
