...
import { getMetroRegionalCopy } from "@/i18n/metro-regional-copy";
import { getTexasLocationCopy } from "@/i18n/texas-location-copy";
import {
  getMetroRegionalConfig,
  getMetroRegionalCountyLinks,
...
export function metroRegionalMetadata(metroId: MetroRegionalId, locale: HomeLocale): Metadata {
  const copy = getMetroRegionalCopy(metroId, locale);
  const config = getMetroRegionalConfig(metroId);
  return buildLocalizedMetadata(locale, config.landingPath, {
    title: copy.metaTitle,
    description: copy.metaDescription,
  });
}

export function MetroRegionalLandingPage({
  metroId,
  locale,
}: {
  ...
  const counties = getMetroRegionalCountyLinks(metroId, locale);
