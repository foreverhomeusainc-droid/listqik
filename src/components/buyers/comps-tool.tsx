...
import { CompsAccessGate, useCompsAccess } from "@/components/buyers/comps-access-gate";
...
export function CompsTool({
  ...
  const { access, loading: accessLoading } = useCompsAccess();
  ...
  async function runCompsApi() {
    ...
    try {
      const res = await fetch("/api/buyers/comps", {
        ...
        body: JSON.stringify({ beds, baths, sqft, zip, dealId }),
      });
      ...
      if (!res.ok || !data.ok || !data.result) {
        if (data.code === "comps_premium_required" || res.status === 403) {
          setGateOpen(true);
          return;
        }
  ...
  function onGenerate() {
    if (!accessLoading && !access?.canRunInstant) {
      setGateOpen(true);
      return;
    }
    void runCompsApi();
  ...
  useEffect(() => {
    if (autoRun && initialZip && access?.canRunInstant) {
      void runCompsApi();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps -- auto-run once when premium access confirmed
  }, [autoRun, initialZip, access?.canRunInstant]);
  ...
  return (
    <>
      <div className="rounded-2xl border border-emerald-500/25 bg-black/45 p-5 sm:p-6">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <h2 className="mt-1 text-xl font-semibold text-emerald-50">
              {dealId ? "Per-deal MLS comparables" : "MLS-backed comparables"}
            </h2>
          </div>
        </div>
        ...
        >
          {busy ? "Generating comps…" : "Generate Comps"}
        </button>
