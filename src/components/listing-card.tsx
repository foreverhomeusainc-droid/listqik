...
} from "@/i18n/listings-copy";

function formatMoney(n: number, locale: HomeLocale) {
  return n.toLocaleString(locale === "es" ? "es-US" : undefined, {
    style: "currency",
    currency: "USD",
    ...
    >
      <div className="relative aspect-[16/10] w-full overflow-hidden">
        ...
        <div className="absolute left-4 top-4 flex items-center gap-2">
          <span className="chip">{translateListingStatus(locale, listing.status)}</span>
          <span className="chip">{translateListingType(locale, listing.type)}</span>
        </div>
      </div>
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="text-right">
            <div className="text-xs font-mono text-white/50">{copy.priceLabel}</div>
        ...
        <div className="mt-5 flex items-center justify-between text-xs text-white/60">
          <div className="font-mono">
            {listing.beds ? `${listing.beds} ${copy.bedsAbbr}` : "—"} ·{" "}
            {listing.baths ? `${listing.baths} ${copy.bathsAbbr}` : "—"} ·{" "}
            {listing.sqft
              ? `${listing.sqft.toLocaleString(locale === "es" ? "es-US" : undefined)} ${copy.sqftAbbr}`
              : "—"}
          </div>
